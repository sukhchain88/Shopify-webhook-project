 ...
 */
async function handleOrderCreated(job: Job<WebhookJobData>, shopifyOrder: any): Promise<JobResult> {
  ...
  if (existingOrder) {
    ...
    return {
      ...
      processedAt: new Date(),
      data: { orderId: (existingOrder as any).id, action: 'skipped' },
    };
  }
 ...
 */
async function handleOrderUpdated(job: Job<WebhookJobData>, shopifyOrder: any): Promise<JobResult> {
 ...
 */
async function handleOrderCancelled(job: Job<WebhookJobData>, shopifyOrder: any): Promise<JobResult> {
 ...
 */
async function handleOrderPaid(job: Job<WebhookJobData>, shopifyOrder: any): Promise<JobResult> {
  const order = await Order.findOne({
    where: { shopify_order_id: shopifyOrder.id.toString() }
  });
 ...
 */
async function handleOrderFulfilled(job: Job<WebhookJobData>, shopifyOrder: any): Promise<JobResult> {
  const order = await Order.findOne({
    where: { shopify_order_id: shopifyOrder.id.toString() }
  });
 ...
 */
async function handleProductCreated(job: Job<WebhookJobData>, shopifyProduct: any): Promise<JobResult> {
 ...
 */
async function handleProductUpdated(job: Job<WebhookJobData>, shopifyProduct: any): Promise<JobResult> {
 ...
 */
async function handleProductDeleted(job: Job<WebhookJobData>, shopifyProduct: any): Promise<JobResult> {
 ...
 */
function getWebhookErrorCode(error: unknown, source: string): string {
 ...
 */
function isWebhookRetryable(error: unknown, eventType: string): boolean {
  if (error instanceof Error) {
    const message = error.message.toLowerCase();
